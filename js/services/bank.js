(function() {
    window.initBankService = function() {
    ...
    async function renderBankView(container) {
        ...
        const txlogObj = await window.KP_DB.dbGet(`users/${userNick}/txlog`) || {};
        const loansObj = await window.KP_DB.dbGet(`users/${userNick}/loans`) || {};
    ...
    window.openTransferModal = async function() {
        ...
        const currentUserNick = window.KP.getCurrentNick();

        const recipientOptions = Object.keys(allUsers)
            .filter(key => key !== currentUserNick)
            .map(key => {
                const u = allUsers[key];
                return `<option value="${key}">${window.KP.displayNick(u)} (${key})</option>`;
            }).join('');
    ...
    window.submitTransferFlow = async function() {
        ...
        try {
            const receipt = await window.KP.transferMoney(recipient, amount, desc);
            closeTransferModal();
    ...
    window.submitLoanFlow = async function() {
        ...
        try {
            await window.KP.applyForLoan(amount);
    ...
    window.openRepayLoanModal = function(loanKey, remaining) {
        ...
        if (p) {
            window.KP.repayLoan(loanKey, p).then(() => {
