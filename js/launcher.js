(function() {
    ...
    function navigateTo(serviceId) {
        if (!SERVICES.some(s => s.id === serviceId)) {
            serviceId = 'portal';
        }

        const user = window.KP.getCurrentUser();
        if (serviceId === 'mayor' && (!user || !window.KP.isMayor(user))) {
    ...
    function renderActiveServiceView() {
        const user = window.KP.getCurrentUser();
    ...
    function renderGlobalHeader() {
        ...
        const displayName = window.KP.displayNick(user);
    ...
    window.toggleEcosystemMenu = function() {
        ...
        const user = window.KP.getCurrentUser();
        const list = SERVICES.filter(s => !s.mayorOnly || (user && window.KP.isMayor(user)));
        ...
        modal.innerHTML = `
            <div class="modal-card">
                ...
                <div style="display:grid;grid-template-columns:repeat(2, 1fr);gap:10px;max-height:65vh;overflow-y:auto;padding-right:4px;">
                    ${list.map(s => `
                        <div class="eco-service-card" onclick="window.KP.navigateTo('${s.id}');closeEcosystemMenu();">
    ...
    window.toggleNotificationsModal = function() {
        ...
        const list = window.KP.getNotifications();
    ...
    window._markAllRead = async function() {
        await window.KP.markAllNotificationsAsRead();
        window.toggleNotificationsModal();
    };

    window._clickNotif = async function(id, url) {
        await window.KP.markNotificationAsRead(id);
        ...
        modal.innerHTML = `
            <div class="modal-card">
                ...
                <div style="text-align:center;margin-bottom:16px;">
                    ...
                    <div style="font-size:18px;font-weight:800;margin-top:8px;">${window.KP.displayNick(user)}</div>
    ...
    window._logoutApp = function() {
        window.KP.logoutUser();
    };

    window.KP.getServicesList = getServicesList;
    window.KP.navigateTo = navigateTo;
    window.KP.initLauncher = initLauncher;
})();
