...
export function initBusinessesService() {
...
async function renderBusinessesView(container) {
    ...
    const businessesObj = await dbGet('businesses') || {};
    ...
    container.innerHTML = `
        <div style="margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;">
            ...
            <button class="btn btn-primary" style="padding:8px 14px;font-size:12px;" onclick="openCreateBusinessModal()">➕ Открыть бизнес (${CONFIG.fees.businessCreationFee} ₽)</button>
        </div>
...
window.openCreateBusinessModal = function() {
    ...
    modal.innerHTML = `
        <div class="modal-card">
            ...
            <div style="font-size:12px;color:var(--muted);margin-bottom:12px;">Госпошлина за регистрацию бизнеса составляет <b>${CONFIG.fees.businessCreationFee} ₽</b>.</div>
...
window.submitCreateBusinessFlow = async function() {
    ...
    if ((user.balance || 0) < CONFIG.fees.businessCreationFee) {
        return alert(`Недостаточно средств для уплаты госпошлины (${CONFIG.fees.businessCreationFee} ₽)`);
    }

    await dbUpdate(`users/${userNick}`, { balance: user.balance - CONFIG.fees.businessCreationFee });
    ...
    await dbSet(`businesses/${bizKey}`, bizObj);
...
window.openBusinessMiniSite = async function(bizId) {
    const biz = await dbGet(`businesses/${bizId}`);
    if (!biz) return;
...
window.addBizProductPrompt = async function(bizId) {
    ...
    if (name && price) {
        await dbPush(`businesses/${bizId}/products`, {
            name,
            price: Number(price),
            createdAt: Date.now()
        });
